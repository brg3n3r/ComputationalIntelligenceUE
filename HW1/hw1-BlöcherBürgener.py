#Filename: hw1-BlöcherBürgener.py
...
def position_estimation_bayes(data,nr_anchors,p_anchor,prior_mean,prior_cov,lambdas, p_true):
    ...
    for jj in range(0, 201):
        for ii in range(0,201):
            ...
            likelihood_grid[nonzero_likelihood_idx,jj,ii] = np.prod(lambdas*np.exp(-lambdas*(data[nonzero_likelihood_idx,:]-dist_p)),axis=1)*prior
