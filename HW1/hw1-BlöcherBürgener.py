...
def main():
    ...
    params = parameter_estimation(
        reference_measurement, nr_anchors, p_anchor, p_ref)
    ...
    if scenario == 2:
        ...
        position_estimation_least_squares(
            data, nr_anchors, p_anchor, p_true, False)

    if(scenario == 3):
        # 3) Postion estimation using numerical maximum likelihood
        position_estimation_numerical_ml(
            data, nr_anchors, p_anchor, params, p_true)

        # 4) Position estimation with prior knowledge (we roughly know where to expect the agent)
        # specify the prior distribution
        prior_mean = p_true
        prior_cov = np.eye(2)
        position_estimation_bayes(
            data, nr_anchors, p_anchor, prior_mean, prior_cov, params, p_true)
...
def parameter_estimation(reference_measurement, nr_anchors, p_anchor, p_ref):
    ...
    plt.legend(["True Distance", "Measurement 1",
...
def position_estimation_least_squares(data, nr_anchors, p_anchor, p_true, use_exponential):
    ...
    # starting point chosen from uniform distribution
    ...
    for ii in range(0, nr_samples):
        p_ls[ii, :] = least_squares_GN(
            p_anchor, p_start, data[ii, :], max_iter, tol)
...
def position_estimation_numerical_ml(data, nr_anchors, p_anchor, lambdas, p_true):
    ...
    # each slice contains the grid for one sample
    ...
    for jj in range(0, 201):
        for ii in range(0, 201):
            ...
            likelihood_grid[nonzero_likelihood_idx, jj, ii] = np.prod(
                lambdas*np.exp(-lambdas*(data[nonzero_likelihood_idx, :]-dist_p)), axis=1)
    ...
    max_ji = np.column_stack(np.unravel_index(
        max_idx, likelihood_grid[0, :, :].shape))
...
def position_estimation_bayes(data, nr_anchors, p_anchor, prior_mean, prior_cov, lambdas, p_true):
    ...
    for jj in range(0, 201):
        for ii in range(0, 201):
            ...
            likelihood_grid[nonzero_likelihood_idx, jj, ii] = np.prod(
                lambdas*np.exp(-lambdas*(data[nonzero_likelihood_idx, :]-dist_p))*prior, axis=1)
...
def get_prior(p, prior_mean, prior_cov):
    ...
    prior = 1/(2*np.pi*np.linalg.det(prior_cov)**0.5) * np.exp(-0.5 *
                                                               (((p-prior_mean).dot(prior_cov)).dot(np.transpose(p-prior_mean))))
...
def plot_anchors_and_agent(nr_anchors, p_anchor, p_true, p_ref=None):
    ...
    for i in range(0, nr_anchors):
        ...
        plt.text(p_anchor[i, 0] + 0.2, p_anchor[i, 1] +
                 0.2, r'$p_{a,' + str(i) + '}$')
    plt.plot(p_true[0, 0], p_true[0, 1], 'r*')
